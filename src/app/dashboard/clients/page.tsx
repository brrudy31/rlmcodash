...
export default function ContactsPage() {
  ...
  const openHouseCount = contacts.filter((c) => c.source === 'Open House').length;
    ...
    <div className="p-6 lg:p-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-navy-400 text-sm mt-1">
            {contacts.length} total · {openHouseCount} from open houses · {contacts.filter((c) => c.opted_out_at).length} opted out
          </p>
        </div>
      ...
      <div className="bg-navy-800 rounded-xl border border-navy-700 overflow-hidden">
        ...
        <div className="divide-y divide-navy-750">
          {filtered.length === 0 && (
            <p className="text-center py-12 text-navy-500">
          ...
          {filtered.map((c) => {
            ...
            return (
              <div key={c.id} className="hover:bg-navy-750/40 transition-colors">
