...
export async function GET() {
  ...
  const eastern = new Date(nowEastern);
  const todayDate = eastern.toISOString().split('T')[0]; // YYYY-MM-DD
  const nowHHMM = eastern.toTimeString().slice(0, 5);   // HH:MM

  // Find open houses that ended in the last 20 minutes and haven't had a summary sent
  const { rows: houses } = await db.execute({
    sql: `SELECT * FROM open_houses
          WHERE date = ?
            AND end_time IS NOT NULL
            AND end_time <= ?
            AND end_time >= ?
            AND summary_sent_at IS NULL`,
    args: [todayDate, nowHHMM, subtractMinutes(nowHHMM, 20)],
  });

  if (houses.length === 0) {
    return NextResponse.json({ checked: true, sent: 0 });
  }
  ...
  for (const house of houses) {
    ...
    if (signins.length === 0) {
      // Mark as sent even with no sign-ins so we don't keep checking
      await db.execute({
    ...
    const html = buildSummaryEmail(house, signins);

    const { error } = await resend.emails.send({
      from: fromEmail,
      to: toEmail,
      subject: `Open House Summary — ${house.address} (${formatTime(String(house.end_time))})`,
      html,
    });
...
function subtractMinutes(hhmm: string, minutes: number): string {
  ...
  const clipped = Math.max(total, 0);
    ...
    <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:8px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.1);">
      ...
      <tr>
        <td style="padding:28px 36px;">
          ...
          <p style="margin:28px 0 0;color:#888;font-size:12px;text-align:center;">Sent automatically when the open house ended.</p>
        </td>
      </tr>
    </table>
  </td></tr>
